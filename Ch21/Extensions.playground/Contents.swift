...
typealias Velocity = Double //alias a type
...
extension Car { //extend initializer, this will keep the original initializer
...
extension Car {
    mutating func emptyGas(by amount: Double) {
        precondition(amount <= 1 && amount >= 0, gasLevel >= amount, "Amount to remove must be between 0 and 1.")
        gasLevel -= amount
    }
